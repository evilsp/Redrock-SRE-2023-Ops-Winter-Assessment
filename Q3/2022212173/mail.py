        ...
        with open('./file/receivers_emails.txt', 'r') as f:
            ...
            for i in txt:
                ...
                if re.match("^.+\\@(\\[?)[a-zA-Z0-9\\-\\.]+\\.([a-zA-Z]{2,3}|[0-9]{1,3})(\\]?)$", i) != None:
                    received_email_list.append(i)  # 用append将i添加进接收人列表中
                else:
                    print('邮箱格式有误,请检查')
                    exit()
...
def send_mail(sender, password, receivers_mail_list):
    ...
    msg = MIMEMultipart()
    msg['From'] = sender
    msg['To'] = ','.join(receivers_mail_list)
    msg['Subject'] = input("请输入主题：")
    ...
    part_attach1 = MIMEText(open('./file/text.txt', 'r').read(), 'base64', 'utf-8')
